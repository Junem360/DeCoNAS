...
DEFINE_string("output_dir", "./outputs/x2_finetune", "path of result")
...
DEFINE_integer("patch_size", 64, "patch size in training process")
...
DEFINE_boolean("fine_tune", True, "use when test")
...
def train():
    ...
    with g.as_default():
        ...
        image_data = {}
        image_data["data_nums"] = 1000000
        image_data["total_data_num"] = 1000000
        image_data["file_num"] = 800
        image_data["data_files"] = 800
        meta_data["train"] = image_data
        ...
        images["test"], labels["test"], meta_data["test"] = make_eval_batch(test_file_path, 'x2_small', FLAGS.child_upsample_size)

        if FLAGS.child_fixed_arc is None:
            images["valid_rl"], labels["valid_rl"], meta_data["valid_rl"] = threaded_input_word_pipeline(
                valid_file_path,
                file_patterns=[
                    '*.tfrecord'],
                num_threads=4,
        ...
        controllerInferenceClass = ControllerInferenceNetwork
        ...
        if FLAGS.child_fixed_arc is None:
            ...
            controller_inference_model = controllerInferenceClass(
                while_training=True,
                feature_fusion=FLAGS.rl_search,
                skip_target=FLAGS.controller_skip_target,
                skip_weight=FLAGS.controller_skip_weight,
        ...
        with tf.train.SingularMonitoredSession(
            ...
            while True:
                x_train, y_train = get_random_batch(labels["train"], FLAGS.batch_size, FLAGS.patch_size*FLAGS.child_upsample_size, FLAGS.child_upsample_size)

                x_train = x_train - bgr_mean
                y_train = y_train - bgr_mean
                if FLAGS.controller_training:
                    now_arc = sess.run(controller_model.sample_arc)
                    ...
                    loss, lr, gn, _, input_data, label_data, result_data = sess.run(run_ops, {
                        child_model.now_arc: now_arc, child_model.x_train : x_train, child_model.y_train : y_train})
                else:
                    run_ops = [
                        child_ops["loss"],
                        child_ops["lr"],
                ...
                # cv2.imshow("input",input_data[0])
                # cv2.imshow("label",label_data[0])
                # cv2.imshow("pred", result_data[0])
                # cv2.waitKey()
                ...
                if actual_step % FLAGS.it_per_epoch == 0:
                    if (FLAGS.controller_training and
                        ...
                        for ct_step in range(FLAGS.controller_train_steps):
                            ...
                            cb_reward = calculate_cb_penalty(now_arc,child_model.num_cells, child_model.num_layers,
                                                             child_model.feature_fusion, child_model.out_filters)

                            if baseline == 0 and controller_step == 0:
                                baseline = val_PSNR
                            else:
                                baseline -= (1-controller_model.bl_dec)*(baseline-val_PSNR)
                            run_ops = [
                                controller_ops["loss"],
                            ...
                            loss, entropy, lr, gn, skip, _, reward, log_prob = sess.run(run_ops, {controller_ops["valid_PSNR"]: val_PSNR,
                        ...
                        for _ in range(5):
                            ...
                            for _ in range(num_valid_batches):
                                x_valid, y_valid = sess.run([images["valid"], labels["valid"]])
                            ...
                            print("val_PSNR={:<6.4f}, cb_penalty={:<6.4f}, reward={:<6.4f}".format(val_PSNR, cb_reward, val_PSNR-cb_reward))
                            print("-" * 80)
                    ...
                    if child_model.exist_fixed_arc or FLAGS.use_model is not None:
                        ...
                        for i in range(4):
                            ...
                            PSNRs.append(psnr)
                            print("{} database, test_PSNR={:<6.4f}, test_SSIM = {:<6.4f}".format(test_set[i], psnr, ssim))
